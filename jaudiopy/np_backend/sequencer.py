...
from .buffer import NpAudioBuffer, mix_buffers
...
class StepSequencer:
    ...
    def _resolve(self, ch: str, sound_fn: Union[Callable[..., NpAudioBuffer], Dict[str, Any]]) -> Tuple[Optional[Callable[..., NpAudioBuffer]], float]:
        if ch == ".":
            return None, 0.0
    ...
    def render_pattern(self, pattern: str, sound_fn: Union[Callable[..., NpAudioBuffer], Dict[str, Any]],
                       swing: float = 0.0) -> NpAudioBuffer:
        step_dur = self.step_duration
        total_len = int(len(pattern) * step_dur * self.sr)
        track = NpAudioBuffer.silence(total_len / self.sr, self.sr)
    ...
    def render_kit(self, patterns: Dict[str, str], sounds: Dict[str, Union[Callable[..., NpAudioBuffer], Dict[str, Any]]],
                   bars: int = 1, volumes: Optional[Dict[str, float]] = None, swing: float = 0.0) -> NpAudioBuffer:
        tracks: List[NpAudioBuffer] = []
        names: List[str] = []
...
class MelodySequencer:
    ...
    def render(self, events: List[tuple], instrument_fn: Callable[..., NpAudioBuffer]) -> NpAudioBuffer:
...
class Song:
    def __init__(self, sr: int = 44100):
        self.sr = sr
        self.sections: List[NpAudioBuffer] = []

    def add_section(self, buf: NpAudioBuffer) -> 'Song':
        self.sections.append(buf)
        return self

    def render(self) -> NpAudioBuffer:
        out = NpAudioBuffer.silence(0, self.sr)
