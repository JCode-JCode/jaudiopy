...
from .effects import stereo_widener, haas_widen, loudness_maximizer, saturation, exciter
...
def master_chain(left: NpAudioBuffer, right: NpAudioBuffer,
                 bass_gain: float = 1.5, presence_gain: float = 1.0, air_gain: float = 0.5,
                 comp_threshold: float = -16, comp_ratio: float = 3.0,
                 limiter_ceiling: float = -0.3, target_peak: float = 0.98) -> Tuple[NpAudioBuffer, NpAudioBuffer]:
...
def master_chain_advanced(left: NpAudioBuffer, right: NpAudioBuffer,
                          bass_gain: float = 1.5, presence_gain: float = 1.0, air_gain: float = 0.5,
                          comp_threshold: float = -16, comp_ratio: float = 3.0,
                          ...
                          haas_mix: float = 0.0, haas_delay_ms: float = 15) -> Tuple[NpAudioBuffer, NpAudioBuffer]:
    for buf in (left, right):
        low_shelf(buf, freq=100, gain_db=bass_gain)
        eq_band(buf, freq=2500, gain_db=presence_gain, q=1.0)
        high_shelf(buf, freq=10000, gain_db=air_gain)
...
def master_with_preset(left: NpAudioBuffer, right: NpAudioBuffer, preset: str = "balanced",
                       **overrides) -> Tuple[NpAudioBuffer, NpAudioBuffer]:
